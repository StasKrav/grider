...
type Cell struct {
	Text string
	// Добавляем дополнительные поля для хранения информации о ячейке
	Formula string            // Формула ячейки (если есть)
	Format  map[string]string // Форматирование ячейки (цвет, шрифт и т.д.)
	Type    string            // Тип данных в ячейке (text, number, date и т.д.)
	Comment string            // Комментарий к ячейке
}
